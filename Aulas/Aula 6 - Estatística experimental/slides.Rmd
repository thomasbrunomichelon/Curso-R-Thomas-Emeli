...
Como supomos que os erros são normalmente distribuidos, podemos estimar a média de cada tratamento usando a média geral e o efeito do tratamento:
...
Dessa maneira, **o primeiro nível é usado como base e o efeito dos demais níveis são somados a ele.** Apesar de ser diferente, o resultado é o mesmo.
...
A média geral (linha horizontal) é: $97.2$, esse é nosso $\mu$. Já a média **por** tratamento (ponto vermelho), é: dose 0 = $102.2$; dose 5 = $95.1$; dose 10 = $96.7$; dose 15 = $94.7$. Ou seja, dependendo do tratamento, a média dele é maior ou menor que nossa média geral, o quanto maior ou menor é o nosso **efeito do tratamento** ($T_j$). Por exemplo: o efeito da dose 0 é aumentar $5$ unidades da média geral ($102.2 - 97.2$).
...
$*$ **Cuidado com a terminologia!** Linear models, significa modelo linear e significa que os coeficientes da fórmula são lineares entre eles e não que a relação entre a resposta e o tratamento é necessariamente linear (uma linha reta).
...
4.  **Constant Leverage:** *leverage* é "quanto meu dado influencia no meu modelo". Como estamos medindo a distância (erro) de cada observação em relação ao nosso modelo, podemos ver o quanto esse único erro influencia todo nosso modelo. É uma forma de indentificarmos possíveis outliers.
...
1.  Quando escolhemos algum valor de $\lambda \neq 0$, os dados ($y$) são transformados da seguinte forma:
...
O QM é um nome para **variância** e, assim, o desvio padrão é apenas a sua raiz quadrada. Para experimentos no campo da agronomia, a precisão experimental pode ser classificada conforme o CV(%) (Gomes, 1987):
...
É comum atribuirmos letras na nossa comparação de médias para facilitar a visualização delas: letras iguais/diferentes, médias iguais/diferentes. Para isso, usamos a função **cld()** (*compact letter display*) do pacote **multcomp**:
...
Imagine um experimento em que temos cinco níveis: solo sem adubação, com adubação orgânica 1 e 2; e com adubação química 1 e 2. Além de compararmos as médias duas a duas, poderiamos querer algumas comparações interessantes como:
...
![](images/contrastes2.png){width="753"}
...
$*$ **Cuidado com a terminologia!** Distribuição é a distribuição de probabilidade que é simplesmente a probabilidade de obtermos um determinado valor de algo que medimos várias vezes (como em nossas repetições). Existem algumas distribuições teóricas que usamos, já que mais fácil (e barato) do que realizar milhares de repetições.
...
Uma quarta alternativa é testar alguma outra distribuição, a qual não exige os pressupostos da normalidade:

$$
Média(transformação(dados)) \neq Transformação(média(dados))
$$

$*$ **Atenção!** Outra distribuição pode se ajustar melhor aos dados, **mesmo que esses atendam a normalidade**.
...
media_poisson <- emmeans(modelo_poisson, specs=~lote|peneira, type="response") # estimando as médias modelo Poisson
library(multcomp) # carregando pacote com as "letras"
letras_normal <- cld(media_normal, Letters = LETTERS, reversed = T) # média com as "letras"
letras_poisson <- cld(media_poisson, Letters = LETTERS, reversed = T) # médias com as "letras"
cbind(letras_normal, letras_poisson)[,c(1,2,8,16)] # colocando lado a lado as respostas
library(ggplot2) # carregando ggplot2
library(gridExtra) # carregando pacote para criar painél
g1 <- ggplot(as.data.frame(media_normal))+ # dados
